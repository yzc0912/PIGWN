...
def parse_args(DATASET, parser):
    # get configuration
    config_file = '../conf/PIGWN/{}.conf'.format(DATASET)
    ...
    # new
    parser.add_argument('--original_dim', type=int, default=config['new']['original_dim'])
    parser.add_argument('--gcn_ks', type=int, default=config['new']['gcn_ks'])
    parser.add_argument('--gcn_c_in', type=int, default=config['new']['gcn_c_in'])
    parser.add_argument('--gcn_c_out', type=int, default=config['new']['gcn_c_out'])
    parser.add_argument('--gru_h_dim', type=int, default=config['new']['gru_h_dim'])
    parser.add_argument('--per_step', type=int, default=config['new']['per_step'])
    ...
    elif DATASET == 'china' or DATASET == 'usa':
    # 读取 CSV 文件，跳过第一行（列序号），并从第二列开始读取（跳过行序号）
        A = pd.read_csv(args.filepath + DATASET + '.csv', header=None, skiprows=1, usecols=range(1, int(args.num_nodes)+1)).values.astype(np.float32)
    else:
