...
class New_Trainer(object):
    ...
    def data_loader_generator(self, paths):
        for path in paths:
    ...
    def train_epoch(self, epoch):
        ...
        total_flow_loss = 0
        total_s_loss = 0
        loader_generator = self.data_loader_generator(self.train_loader)
    
        # 迭代生成数据加载器并加载数据
        for dataloader in loader_generator:
            allocated = torch.cuda.memory_allocated()
            print(f"在train之前Allocated GPU memory: {allocated / 1024**3:.2f} GB")
            for (data, data_mat ,target, phy_target) in dataloader:
                self.batch_seen += 1
                data = data[..., :self.args.input_base_dim]
                label = target[..., :]
    ...
    def train(self):
        ...
        for epoch in range(1, self.args.epochs + 1):
            ...
            allocated = torch.cuda.memory_allocated()
            print(f"在train之后Allocated GPU memory: {allocated / 1024**3:.2f} GB")
            ...
            if best_state == True:
                ...
                model_save_path = f"./{self.args.model}/{self.args.dataset}_1/{self.args.dataset}_1.pth"
